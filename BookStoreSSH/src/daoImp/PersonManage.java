...
public class PersonManage extends HibernateDaoSupport implements PersonDao {

	// 管理员登录验证
	@SuppressWarnings("unchecked")
	public boolean checkManager(String managerName, String managerPassword) {
		boolean flag = false;
		String hql = "from Manager as manager where manager.managerName = '"
				+ managerName + "' and manager.managerPassword = '"
				+ managerPassword + "'";
	...
	public User findUser(int userId) {
		try {
			User user = (User) this.getHibernateTemplate().get("entity.User",
					userId);
			return user;
	...
	public Sex findSex(int sexId) {
		try {
			Sex sex = (Sex) this.getHibernateTemplate()
					.get("entity.Sex", sexId);
			return sex;
		} catch (RuntimeException re) {
			throw re;
		}
