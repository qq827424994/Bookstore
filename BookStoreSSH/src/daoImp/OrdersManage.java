...
public class OrdersManage extends HibernateDaoSupport implements OrdersDao{
	// 查询所有订单
	@SuppressWarnings("unchecked")
	public List<Orders> allOrders(final int pageNumber, final int pageSize) {
		List<Orders> allOrders = getHibernateTemplate().executeFind(
	...
	public List<Orders> allOrdersByDeal(final String isDeal,
		...
		List<Orders> allOrders = getHibernateTemplate().executeFind(
				new HibernateCallback() {
					public Object doInHibernate(Session session)
							throws HibernateException, SQLException {
						String hql = "from Orders as orders where orders.isDeal = '"
								+ isDeal + "' order by ordersTime desc";
						Query query = session.createQuery(hql);
						query.setFirstResult((pageNumber - 1) * pageSize);
						query.setMaxResults(pageSize);
						List<Orders> list = query.list();
	...
	public int deleteOrders(int ordersId) {
		...
		try {
			this.getHibernateTemplate().delete(
					this.getHibernateTemplate().get("entity.Orders", ordersId));
			i = 1;
		} catch (RuntimeException re) {
			throw re;
		}
	...
	public Orders findOrders(int ordersId) {
		try {
			Orders orders = (Orders) this.getHibernateTemplate().get(
					"entity.Orders", ordersId);
			return orders;
		} catch (RuntimeException re) {
			throw re;
		}
	...
	public List<Orders> allOrdersByUser(int userId, int pageNumber, int pageSize) {
		String hql = "from Orders as orders where orders.user.userId=" + userId
				+ " order by ordersTime desc";
		try {
			List<Orders> allOrdersByUser = this.getHibernateTemplate()
					.find(hql);
	...
	public List<Ordersbook> allOrdersbookByOrders(int ordersId) {
		String hql = "from Ordersbook as ordersbook where ordersbook.orders.ordersId="
				+ ordersId;
		try {
			List<Ordersbook> allOrdersbookByOrders = this
